...
3. Попробуйте подключиться к Wi-Fi (убедитесь что он работает). Если с Wi-Fi всё работает, а на мобильном интернете нет - скорее всего в вашем регионе сейчас белые списки (БС). Часто их можно обойти, см. "ОБХОД БЕЛЫХ СПИСКОВ (БС)".
...
### [НОВОЕ]: ОБХОД БЕЛЫХ СПИСКОВ (БС)

> Сервер был временно отключен на неопределённый срок. Поддерживать сервера такого типа гораздо сложнее чем обычные, но когда-нибудь он будет добавлен снова.

Рад сообщить, что с недавнего времени в подписке доступен новый сервер "Польша БС". Этот сервер особенный, потому что он позволяет обходить белые списки. Работает не всегда, но весьмы часто -- проверял сам.

Важно также что при использовании этого сервиса лимит трафика расходуется x100, так что убедительно прошу использовать его **только и только тогда, когда есть БС**.

#### Как понять, есть ли белые списки

Самый простой способ - попытаться открыть эту инструкцию. Если она открывается, значит белых списков нет. Если не открывается (и это не обусловлено прочими проблемами Вашего соединения) -- БС скорее всего есть.
